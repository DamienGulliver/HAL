...
# Add constant for the FIFO path
...
def record_audio(stop_event):
    ...
    try:
        ...
        print(f"[INFO] Created FIFO at {AUDIO_FIFO}")
    except Exception as e:
        print(f"[ERROR] Failed to create FIFO: {e}")
        return

    # Create a pseudo-terminal for capturing arecord output
    master, slave = pty.openpty()
    
    print("[INFO] Starting arecord process...")
    process = subprocess.Popen([
        'arecord', '-D', 'softvol', 
        '-f', 'S16_LE',      # 16-bit signed little-endian
        '-r', '16000',       # 16kHz sample rate for OpenAI
        '-c', '1',           # Mono audio
        '-V', 'stereo',      # VU meters
        '--buffer-size=1024', # Small buffer for lower latency
    ...
    # Open master as file for VU meter output
    master_file = os.fdopen(master, 'r')
    
    # Open FIFO for writing
    try:
        print("[INFO] Opening FIFO for writing...")
        fifo = open(AUDIO_FIFO, 'wb')
    except Exception as e:
        print(f"[ERROR] Failed to open FIFO: {e}")
        process.terminate()
        return
    ...
    try:
        while not stop_event.is_set():
            ...
            if data:
                ...
                try:
                    ...
                    if e.errno == 32:  # Broken pipe
                        print("[WARNING] Broken pipe - waiting for reader...")
                        time.sleep(0.1)
                        continue
                    raise
            ...
            try:
                vu_output = master_file.readline()
                if vu_output:
                    sys.stdout.write('\r' + vu_output.strip())
                    sys.stdout.flush()
            ...
            if process.poll() is not None:
                print("[WARNING] arecord process died, restarting...")
                process = subprocess.Popen([
                    'arecord', '-D', 'softvol', 
                    '-f', 'S16_LE',
                    '-r', '16000',
        ...
        print("\n[INFO] Cleaning up audio recording...")
        process.terminate()
        fifo.close()
        master_file.close()
        if os.path.exists(AUDIO_FIFO):
...
def run_face_detection(stop_event):
    # Common locations for Haar cascade file on Jetson Nano
    cascade_paths = [
    ...
    try:
        ...
        print("[INFO] Cleaning up video stream...")
        vs.stop()
